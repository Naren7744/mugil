...
import { FiPlus, FiMinus, FiArrowRight, FiPercent, FiAward, FiShield, FiChevronDown } from 'react-icons/fi';
...
const Membership = () => {
  ...
  data-aos-duration="1000" className="max-w-[1400px] mx-auto space-y-4">
          ...
          <div className="inline-flex items-center gap-2 bg-[#0a0a0a] border border-zinc-800 px-4 py-1.5 shadow-sm">
            ...
            <span className="text-[#ffc114] text-[9px] font-bold tracking-[0.35em] uppercase">Membership Plans</span>
          </div>
          ...
          <p className="text-zinc-500 text-xs sm:text-sm font-light max-w-xl mx-auto leading-relaxed">
            Simple, honest plans built around your goals. Pick a duration, see what's included, and get started.
          </p>
            ...
            {durations.map((dur) => (
              <button
                key={dur}
                  data-aos="zoom-in"
  data-aos-delay={100}
                onClick={() => { setSelectedDuration(dur); setActiveDetailPlan(null); }}
      ...
      <section className="py-16 sm:py-24 px-4 sm:px-6 md:px-12 lg:px-24 bg-[#030303]">
        <div className="max-w-[1300px] mx-auto space-y-6">
          ...
          {/* Loading state */}
          {loading && (
            <div className="text-center py-20 text-zinc-500 text-sm">Loading plans...</div>
          )}

          {/* Empty state */}
          {!loading && visiblePlans.length === 0 && (
            <div className="text-center py-20 text-zinc-500 text-sm">
              No plans available for {selectedDuration} right now. Try another duration.
            </div>
          )}
  ...
  data-aos-duration="800"
              ...
              >
                ...
                >
                  ...
                  <div className="space-y-2 max-w-md w-full">
                    ...
                    <p className="text-zinc-500 text-xs sm:text-sm font-light leading-relaxed">
                      {plan.description || `${plan.planType} · ${plan.duration}`}
                    </p>
                  </div>

                  {/* Center: feature tags (desktop only) */}
                    ...
                    <div className="text-left lg:text-right">
                      ...
                      <span className="text-[10px] sm:text-[11px] font-medium tracking-wide block mt-0.5 text-zinc-500">
                        {admissionFee > 0 ? (
                          ...
                          <span className="text-emerald-500 font-bold uppercase">No joining fee</span>
                        )}
                      </span>
                    </div>
                ...
                {/* Expandable details */}
                <div
                   data-aos="fade-up"className={`transition-all duration-500 ease-in-out bg-black border-t border-zinc-900/60 overflow-hidden ${
                    isExpanded ? 'max-h-[1200px] opacity-100 p-5 sm:p-10' : 'max-h-0 opacity-0 pointer-events-none'
                  }`}
                >
                    ...
                    <div className="lg:col-span-8">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3.5">
                        {plan.features?.map((feat, fIdx) => (
                          <div key={fIdx} className="flex items-start gap-2.5 text-[13px] text-zinc-400 font-light">
                            <span className="text-[#ffc114] font-bold mt-0.5">✓</span>
                            <span>{feat}</span>
                          </div>
                        ))}
                      </div>

                      {/* Extra boolean flags from backend */}
                        ...
                        <div className="text-zinc-500 text-xs space-y-2">
                          ...
                          {admissionFee > 0 && (
                            <div className="flex justify-between">
                              ...
                              <span className="text-white font-bold">₹{admissionFee.toLocaleString('en-IN')}</span>
                            </div>
                          )}
                          {isOffer && (
                            <div className="flex justify-between text-emerald-500">
                              <span>You save</span>
                              <span className="font-bold">₹{(plan.price - plan.offerPrice).toLocaleString('en-IN')}</span>
                            </div>
                          )}
                          <div className="flex justify-between border-t border-zinc-900 pt-2">
                            <span className="text-zinc-300 font-bold">Total</span>
                            <span className="text-[#ffc114] font-bold">₹{totalCost.toLocaleString('en-IN')}</span>
                          </div>
                        </div>
                      </div>

                      <button   data-aos="zoom-in"
  data-aos-delay="200" className="w-full bg-[#ffc114] text-black hover:bg-white font-bold text-[10px] tracking-[0.25em] py-4 rounded-none uppercase transition-all flex items-center justify-center gap-2 cursor-pointer shadow-xl shadow-yellow-500/5">
                        <span>Join Now</span>
                        <FiArrowRight />
                      </button>
                    </div>
      ...
      {/* ── SECTION 4: TRUST POINTS ───────────────────────────────────────── */}
      <section className="py-20 px-6 md:px-12 lg:px-24 bg-[#030303] select-none">
        <div   data-aos="fade-up" className="max-w-[1300px] mx-auto grid grid-cols-1 md:grid-cols-3 gap-10">

          <div   data-aos="flip-left"
  data-aos-delay="100" className="flex items-start gap-4">
            ...
            <div className="space-y-1">
              <h4 className="text-white font-bold text-xs tracking-wider uppercase">Save on longer plans</h4>
              <p className="text-zinc-500 text-xs font-light leading-relaxed">
                Sign up for 3 or 6 months and the ₹250 joining fee is waived completely.
              </p>
            </div>
          </div>

          <div  data-aos="flip-left"
  data-aos-delay="250" className="flex items-start gap-4">
            <div className="w-10 h-10 bg-[#0a0a0a] border border-zinc-900 flex items-center justify-center text-[#ffc114] shrink-0 shadow-lg">
              <FiAward className="text-base" />
            </div>
            <div className="space-y-1">
              <h4 className="text-white font-bold text-xs tracking-wider uppercase">Full equipment access</h4>
              <p className="text-zinc-500 text-xs font-light leading-relaxed">
                Every plan gives you access to all machines, free weights, and training areas.
              </p>
            </div>
          </div>

          <div   data-aos="flip-left"
  data-aos-delay="400" className="flex items-start gap-4">
            <div className="w-10 h-10 bg-[#0a0a0a] border border-zinc-900 flex items-center justify-center text-[#ffc114] shrink-0 shadow-lg">
              <FiShield className="text-base" />
            </div>
            <div className="space-y-1">
              <h4 className="text-white font-bold text-xs tracking-wider uppercase">No hidden fees</h4>
              <p className="text-zinc-500 text-xs font-light leading-relaxed">
      ...
      <section className="py-24 px-6 md:px-12 lg:px-24 bg-[#060606] border-t border-b border-zinc-900/60">
        <div className="max-w-[1000px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div  data-aos="fade-right" className="lg:col-span-4 space-y-2 select-none">
            <span className="text-[#ffc114] text-[10px] font-bold tracking-[0.3em] uppercase block">// Questions</span>
            <h2 className="text-white font-gym-brutal text-2xl uppercase tracking-tight">Common FAQs</h2>
            <p className="text-zinc-500 text-xs font-light leading-relaxed">Everything you need to know before signing up.</p>
          </div>

          <div  data-aos="fade-left" className="lg:col-span-8 divide-y divide-zinc-900 border-t border-b border-zinc-900 bg-[#0a0a0a] shadow-2xl">
            {faqs.map((faq, fIdx) => {
  ...
  data-aos-delay={fIdx * 50}
                ...
                >
                  <div className="flex justify-between items-center gap-4">
                    <h4 className="text-white font-bold text-xs tracking-wide">{faq.q}</h4>
                    <FiChevronDown
                  ...
                  <div
                    className={`transition-all duration-500 overflow-hidden ${
                      isOpen ? 'max-h-40 opacity-100 mt-3 pt-3 border-t border-zinc-900' : 'max-h-0 opacity-0 pointer-events-none'
                    }`}
                  >
                    <p className="text-zinc-500 text-xs sm:text-sm font-light leading-relaxed">{faq.a}</p>
                  </div>
                </div>
              );
            })}
  ...
  data-aos-duration="1000" className="max-w-[900px] mx-auto bg-[#0a0a0a] border border-zinc-900 p-8 sm:p-12 text-center relative shadow-2xl">
          ...
          <p className="text-zinc-500 text-xs sm:text-sm max-w-xl mx-auto font-light leading-relaxed mb-8">
            Want bulk memberships for your team or company? We'll put together a custom plan. Get in touch and we'll handle the rest.
          </p>

          <div    data-aos="fade-up"
  data-aos-delay="200" className="flex flex-col sm:flex-row items-center justify-center gap-4 text-[10px] font-bold tracking-widest uppercase">
            <a
            ...
            <button
              onClick={() => { setSelectedDuration('3 Months'); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
