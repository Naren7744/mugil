...
const HIDDEN_FIELDS = [
  ...
  "isDeleted", "paymentHistory", "photo", "password",
];
...
const COLOR_MAP = {
  amber:   { border: "hover:border-amber-300",   shadow: "hover:shadow-amber-500/5",   icon: "bg-amber-50 text-amber-500",     text: "text-amber-500" },
  red:     { border: "hover:border-red-300",     shadow: "hover:shadow-red-500/5",     icon: "bg-red-50 text-red-500",         text: "text-red-500" },
  emerald: { border: "hover:border-emerald-300", shadow: "hover:shadow-emerald-500/5", icon: "bg-emerald-50 text-emerald-500", text: "text-emerald-500" },
  cyan:    { border: "hover:border-cyan-300",    shadow: "hover:shadow-cyan-500/5",    icon: "bg-cyan-50 text-sky-500",        text: "text-cyan-500" },
  violet:  { border: "hover:border-violet-300",  shadow: "hover:shadow-violet-500/5",  icon: "bg-violet-50 text-violet-500",   text: "text-violet-500" },
  sky:     { border: "hover:border-sky-300",     shadow: "hover:shadow-sky-500/5",     icon: "bg-sky-50 text-sky-500",         text: "text-sky-500" },
};
...
const buildSummaryBoxes = (rows, reportType, period = "today") => {
  ...
  if (reportType === "pending-fees") {
    ...
    // rows are already sorted by balanceAmount desc (backend: .sort({ balanceAmount: -1 }))
    const highestBalance = rows.length > 0 ? Number(rows[0].balanceAmount || 0) : 0;

    return [
      {
        label: "Members with Due",
        value: `${totalMembers}`,
        bgColor: [230, 241, 251], border: [133, 183, 235],
...
const activeMembers = rows.filter((member) => {
  ...
  // ── BRANCH REVENUE / ACTIVE BRANCHES ──────────────────────────────────────
  // NOTE: active-branches API returns: branchName, branchCode, ownerName, status,
  //       totalMembers, totalRevenue — NO pendingAmount field.
  //       branch-revenue API returns:  branchCode, branchName, ownerName,
  //       totalMembers, activeMembers, totalRevenue, pendingAmount.
  if (reportType === "branch-revenue" || reportType === "active-branches") {
    const totalBranches  = rows.length;
    const totalMembers   = rows.reduce((s, b) => s + Number(b.totalMembers || 0), 0);
    const totalRevenue   = rows.reduce((s, b) => s + Number(b.totalRevenue || 0), 0);
    // pendingAmount only exists in branch-revenue; active-branches has no pending field
    const totalPending   = rows.reduce((s, b) => s + Number(b.pendingAmount || 0), 0);
    // activeMembers only exists in branch-revenue
    const totalActive    = rows.reduce((s, b) => s + Number(b.activeMembers || 0), 0);

    // Both active-branches and branch-revenue now return pendingAmount (backend fixed).
    // active-branches does NOT return activeMembers — use totalMembers for that type.
    // branch-revenue returns activeMembers — use it there.
...
if (reportType === "today-collection") {
  ...
  // ── FALLBACK (generic) ─────────────────────────────────────────────────────
  const totalMembers   = rows.length;
  const totalCollected = rows.reduce((s, m) => s + Number(m.amountPaid || m.amount || m.totalRevenue || 0), 0);
  const totalBalance   = rows.reduce((s, m) => s + Number(m.balanceAmount || m.pendingAmount || 0), 0);
  const expiredCount   = rows.filter((m) => m.expiryDate && new Date(m.expiryDate) < today).length;
...
const buildPDF = (rows, title, branch, period, reportType = "") => {
  ...
  doc.setFontSize(9);
  doc.setTextColor(148, 163, 184);
  doc.text(`Date: ${new Date().toLocaleDateString("en-IN")}`, pageW - 14, 30, { align: "right" });
  ...
  const branchLabel = branch === "ALL_BRANCHES" ? "All Branches" : (branch || "All Branches");
  doc.setFont("helvetica", "normal");
  doc.setFontSize(9);
...
const boxes = buildSummaryBoxes(
  ...
  const headers = columns.map(
    (k) => COLUMN_LABELS[k] || k.replace(/([A-Z])/g, " $1").replace(/^./, (s) => s.toUpperCase())
  );
...
function Reports() {
  ...
  const [metrics, setMetrics] = useState({
    totalRevenue: 0, pendingPayments: 0, activeMembers: 0,
    expiringMembers: 0, branchesReporting: 0, todayCollection: 0, newMembers: 0,
  });
  ...
  // ── Open modal ────────────────────────────────────────────────────────────────
  // FIXED: renamed "api" parameter to "endpoint" to avoid shadowing the imported api instance
  const openReportModal = useCallback(async (title, endpoint, type = "") => {
  ...
  // ── PDF from modal ────────────────────────────────────────────────────────────
  const downloadReportPDF = useCallback((rows = []) => {
buildPDF(
    rows,
    reportTitle,
    currentBranch,
  ...
  const handleDownloadReport = useCallback(async (card) => {
    setDownloadingId(card.id);
    try {
      const res = await api.get(
        card.api.replace("/api/v1", ""),
        { params: { branch: currentBranch, period: dateRange } }
      );
...
const subText =
  ...
  {typeof card.label === "function"
    ...
    : card.label}
                  ...
                  <p className={`text-[11px] font-medium ${c.text}`}>{subText}</p>
                </div>
                <div className={`w-11 h-11 rounded-xl ${c.icon} flex items-center justify-center group-hover:scale-110 transition-transform duration-200`}>
                  {card.icon}
                </div>
              </div>
            );
            ...
            <div className="h-56 w-full" style={{ minWidth: 0 }}>
              {chartsLoading ? (
                ...
                <ResponsiveContainer width="100%" height={224}>
                  <LineChart data={revenueOverviewData} margin={{ top: 8, right: 5, left: -20, bottom: 5 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
                    <XAxis dataKey="day" tick={{ fontSize: 10, fill: "#94a3b8", fontWeight: 700 }} axisLine={false} tickLine={false} dy={10} />
                    <YAxis tick={{ fontSize: 10, fill: "#94a3b8", fontWeight: 700 }} axisLine={false} tickLine={false} tickFormatter={(v) => `₹${(v / 1000).toFixed(0)}K`} />
                    <Tooltip content={<ChartTooltip prefix="₹" />} />
                    <Line type="monotone" dataKey="thisWeek" stroke="#6366f1" strokeWidth={3} dot={{ r: 4, fill: "#6366f1", stroke: "#fff", strokeWidth: 2 }} activeDot={{ r: 6, strokeWidth: 0, fill: "#4f46e5" }} />
...
: "Overall Membership"
                ...
                <div className="flex items-center gap-4 text-[11px] font-semibold text-slate-400 mt-3">
                  <span className="flex items-center gap-2 text-slate-600">
                    <span className="w-3 h-2.5 bg-[#6366f1] rounded-sm" /> Active 
                  </span>
                  <span className="flex items-center gap-2 text-slate-600">
            ...
            <div className="w-full h-56 min-w-0 min-h-[224px]">
              {chartsLoading ? (
                ...
                <ResponsiveContainer width="100%" height={224}>
                  <BarChart data={membershipGrowthData} margin={{ top: 8, right: 5, left: -20, bottom: 5 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
                    <XAxis dataKey="day" tick={{ fontSize: 10, fill: "#94a3b8", fontWeight: 700 }} axisLine={false} tickLine={false} dy={10} />
                    <YAxis tick={{ fontSize: 10, fill: "#94a3b8", fontWeight: 700 }} axisLine={false} tickLine={false} />
                    <Tooltip content={<ChartTooltip />} />
                    <Bar   dataKey="totalMembers" fill="#6366f1" radius={[6, 6, 0, 0]} barSize={10} />
                    <Line type="monotone" dataKey="newMembers" stroke="#10b981" strokeWidth={2.5} dot={{ r: 3, fill: "#10b981", stroke: "#fff", strokeWidth: 1 }} />
                  </BarChart>
                </ResponsiveContainer>
              )}
            </div>
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 bg-slate-50/60 border border-slate-200/50 rounded-[1.75rem] p-5">
            <div className="space-y-1">
              ...
              <p className="text-[11px] text-slate-400 font-semibold tracking-wide">
                Click Preview to view data first, or Download PDF to save directly.
              </p>
            </div>
            <div className="flex items-center gap-3 bg-white border border-slate-200/60 rounded-xl px-3.5 py-1.5">
              <span className="text-[9px] uppercase font-black text-slate-400 tracking-wider font-mono">Period</span>
              <span className="h-3 w-[1px] bg-slate-200" />
              <span className="text-[10px] font-mono font-black text-sky-600">{dateRange}</span>
            </div>
          </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-5 pt-4 border-t border-slate-100 flex items-center justify-between gap-4">
                    <button
                      onClick={() => openReportModal(card.name, card.api, card.type)}
                      type="button"
                    ...
                    >
                      {isDownloading ? (
                        <><FiLoader size={12} className="animate-spin" /> Generating...</>
                      ) : (
