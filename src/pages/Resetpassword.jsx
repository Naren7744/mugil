...
import { FiLock, FiEye, FiEyeOff, FiCheckCircle, FiActivity, FiAlertTriangle } from 'react-icons/fi';
        ...
        <div className="bg-[#0d0d11]/80 backdrop-blur-xl border border-neutral-900 rounded-[2rem] p-8 shadow-[0_40px_80px_rgba(0,0,0,0.8)] relative overflow-hidden">
          ...
          {success ? (
            <div className="flex flex-col items-center text-center gap-4 py-4">
              ...
              <div>
                <h2 className="text-base font-bold text-white mb-2">Password Updated</h2>
            ...
            <>
              <div className="mb-6">
                <h2 className="text-lg font-bold text-white tracking-tight mb-1">Set New Password</h2>
                <p className="text-neutral-500 text-xs leading-relaxed">
              ...
              {error && (
                <div className="mb-4 p-3 bg-[#ff4a4a]/5 border border-[#ff4a4a]/10 rounded-xl flex items-start gap-2.5">
                  <FiAlertTriangle className="text-[#ff4a4a] mt-0.5 shrink-0" size={14} />
                  <div>
                    <p className="text-[#ff4a4a] text-xs font-medium">{error}</p>
                    <button
                      onClick={() => navigate('/login')}
              ...
              <form onSubmit={handleReset} className="space-y-4" noValidate>
                ...
                <div className="space-y-1.5">
                  ...
                  {fieldErrors.newPassword && (
                    <p className="text-red-400 text-[10px] pl-1 font-medium">{fieldErrors.newPassword}</p>
                  )}
                  {/* Strength bar */}
                  {newPassword && !fieldErrors.newPassword && (
                    <div className="pt-1 flex items-center gap-2 px-1">
                      <div className="flex gap-1 flex-1">
                        <div className={`h-[2px] flex-1 rounded-full transition-all duration-500 ${getStrength() >= 1 ? 'bg-[#ff4a4a]' : 'bg-neutral-900'}`} />
                        <div className={`h-[2px] flex-1 rounded-full transition-all duration-500 ${getStrength() >= 2 ? 'bg-[#ff4a4a]' : 'bg-neutral-900'}`} />
                        <div className={`h-[2px] flex-1 rounded-full transition-all duration-500 ${getStrength() >= 3 ? 'bg-emerald-500' : 'bg-neutral-900'}`} />
                      </div>
                ...
                <div className="space-y-1.5">
                  ...
                  {fieldErrors.confirmPassword && (
                    <p className="text-red-400 text-[10px] pl-1 font-medium">{fieldErrors.confirmPassword}</p>
                  )}
                  {/* Match indicator */}
                  {confirmPassword && newPassword && !fieldErrors.confirmPassword && (
                    <p className={`text-[10px] pl-1 font-medium ${newPassword === confirmPassword ? 'text-emerald-400' : 'text-red-400'}`}>
